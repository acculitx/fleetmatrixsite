		...
		if($actiondata->enabled == 1 && !empty($files_config)){
			...
			foreach($files_array as $file_string){
				if(strpos($file_string, ':') !== false){
					...
					//mask the file name
					if(strlen($params->get('forced_file_name', '')) > 0){
						$file_name = str_replace('FILE_NAME', $file_name, $params->get('forced_file_name', ''));
					}else{
					...
					if($file_tmp_name){
						...
						if(($file_post["size"] / 1024) > (int)$params->get('max_size', 100)){
							...
							if($uploaded_file){
								$form->files[$file_data[0]] = array('name' => $file_name, 'real_name' => $real_file_name, 'path' => $upload_path.$file_name, 'size' => $file_post["size"]);
