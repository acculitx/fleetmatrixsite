				...
				if(isset($tag['enable_dynamic_data']) && ($tag['enable_dynamic_data'] == 1)){
					if(!empty($tag['data_path']) && !empty($tag['value_key']) && !empty($tag['text_key'])){
						...
						if(!is_null($options_data) && is_array($options_data)){
							...
							foreach($options_data as $option_data){
								if(isset($option_data["'.$tag["value_key"].'"]) && isset($option_data["'.$tag["text_key"].'"])){
									echo \'<input type="'.$fieldoptions['type'].'" name="'.$fieldname.'" id="'.$this->slug($fieldname.'-').'\'.$f_id.\'" title="'.$tag['title'].'" value="\'.$option_data["'.$tag["value_key"].'"].\'"\'.(($option_data["'.$tag["value_key"].'"] == '.var_export($checked_value, true).') ? \' checked="checked"\' : \'\').\' />\'."\n";
					...
					$f_id = 0;
					foreach($options as $k => $option){
						$output .= '<input type="'.$fieldoptions['type'].'" name="'.$fieldname.'" id="'.$this->slug($fieldname.'-'.$f_id).'" title="'.$tag['title'].'" value="'.$k.'"'.(($k == $checked_value) ? ' checked="checked"' : '').' />'."\n";
				...
				if(isset($tag['enable_dynamic_data']) && ($tag['enable_dynamic_data'] == 1)){
					if(!empty($tag['data_path']) && !empty($tag['value_key']) && !empty($tag['text_key'])){
						...
						if(!is_null($options_data) && is_array($options_data)){
							...
							foreach($options_data as $option_data){
								if(isset($option_data["'.$tag["value_key"].'"]) && isset($option_data["'.$tag["text_key"].'"])){
									echo \'<input type="'.$fieldoptions['type'].'" name="'.$fieldname.'[]" id="'.$this->slug($fieldname.'-').'\'.$f_id.\'" title="'.$tag['title'].'" value="\'.$option_data["'.$tag["value_key"].'"].\'"\'.(in_array($option_data["'.$tag["value_key"].'"], '.var_export($checked, true).') ? \' checked="checked"\' : \'\').\' />\'."\n";
									echo \'<label for="'.$this->slug($fieldname.'-').'\'.$f_id.\'">\'.$option_data["'.$tag["text_key"].'"].\'</label>\'."\n";
									$f_id++;
								}
							}
					...
					$f_id = 0;
					foreach($options as $k => $option){
						$output .= '<input type="'.$fieldoptions['type'].'" name="'.$fieldname.'[]" id="'.$this->slug($fieldname.'-'.$f_id).'" title="'.$tag['title'].'" value="'.$k.'"'.(in_array($k, $checked) ? ' checked="checked"' : '').' />'."\n";
						$output .= '<label for="'.$this->slug($fieldname.'-'.$f_id).'">'.$option.'</label>'."\n";
						$f_id++;
					}
				}
